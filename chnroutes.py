...
def generate_ovpn(metric):
    ...
    print "Usage: Append the content of the newly created routes.txt to your openvpn config file," \
          " and also add 'max-routes %d', which takes a line, to the head of the file." % (
              len(results)+20)


def generate_ros():
    results = fetch_ip_data()
    rsc_header = textwrap.dedent("""\
        /ip firewall address-list remove [/ip firewall address-list find list="cn"]

    """)

    rfile = open('chroutes.rsc', 'w')
    rfile.write(rsc_header)

    for ip, _, mask, comment in results:
        if len(comment) > 0:
            comment = ' comment="' + comment + '"'

        ## route_item = "/ip firewall address-list add list=cn address=%s/%s%s\n" % (
        route_item = "do { /ip firewall address-list add list=cn address=%s/%s%s } on-error={}\n"  % (
            ip, mask, comment)
        rfile.write(route_item)

    rfile.close()
    print "Usage: Upload chroutes.rsc to ROS file system use winbox / webfig (in file menu) \n" \
          "       Use Terminal execute \"import chroutes.rsc\" it will create an IP Address lists named \"cn\"."
...
def fetch_ip_data():
    # with additional file
    myfile = args.inclue

    results = []

    if os.path.isfile(myfile):
        print "- use additional file " + myfile
        ...
        with open(myfile) as my_file:
            ...
            for item in items:
                ...
                if len(item) > 0:
                    ...
                    cird = unit_items[0].split('/')
                    if len(cird) < 2:
                        cird.append(32)
                    if len(unit_items) > 1:
                        results.append(
                            (cird[0], '', cird[1], unit_items[1]))  # comment
                    else:
                        results.append((cird[0], '', cird[1], ''))

    # fetch data from apnic
    print "Fetching data from apnic.net, it might take a few minutes, please wait..."
    url = r'https://ftp.apnic.net/apnic/stats/apnic/delegated-apnic-latest'
    # url = r'http://192.168.21.185/delegated-apnic-latest'
    ...
    # last version info
    metaregex = re.compile(
        r"\d+|\|apnic|\d+\|\d+\|\d+\|\d+\|\d+\|.*", re.IGNORECASE)
    metadata = metaregex.findall(data)

    last_allocation_date = metadata[5]
    ...
    print "- fetch " + str(len(results)) + " items"
    print "- last update " + last_allocation_date
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--platform',
                        ...
                        "win, android, openvpn, ros, ros by default.")
    ...
    parser.add_argument('-i', '--inclue',
                        dest='inclue',
                        default="",
                        nargs='?',
                        type=str,
                        help="Additional ip address for the route rules")
